...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  const getCurrentSection = () => {
    ...
    // If no section is found, return the closest section
    return sections.reduce((closest, current) => {
      if (!current || !closest) return current;
      const currentDistance = Math.abs(current.top - scrollPosition);
      const closestDistance = Math.abs(closest.top - scrollPosition);
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (newIndex !== activeIndex) {
        ...
        const activeLi = navRef.current?.querySelectorAll("li")[newIndex] as HTMLElement;
        if (activeLi) {
